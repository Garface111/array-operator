...
(function(){
  ...
  function loadRenames(){
    return _lsReadJSON(RENAME_KEY, raw => {
      const v = (raw == null ? null : JSON.parse(raw)) || {};
  ...
  function saveRename(kind, id, name){
    ...
    // Build a fresh object (don't mutate the cached read result in place) so a
    // concurrent read sees a consistent value, then invalidate the parse cache.
    const cur = loadRenames();
    const all = { arrays: { ...cur.arrays }, inverters: { ...cur.inverters } };
    const bucket = all[kind] || (all[kind] = {});
    const v = String(name==null ? "" : name).trim();
    if(v) bucket[String(id)] = v; else delete bucket[String(id)];
    try { localStorage.setItem(RENAME_KEY, JSON.stringify(all)); _lsInvalidate(RENAME_KEY); } catch(e){}
  }
